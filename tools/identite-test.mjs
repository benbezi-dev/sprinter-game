...
// ------------------------------------------------------ un nom et son code
console.log('\n── LE NOM ───────────────────────────────────────────────────');
const NOM = `Testeur ${marque}`.slice(0, 20);
...
const sosie = await post('/claim', { device_id: appareil('sosie'), name: 'K7X2QP' });
ok('un pseudo en forme de code passe quand meme',
...
const nomInconnu = await post('/recuperation', { device_id: telPerdu, name: 'personne ' + marque });
ok('on ne recupere pas un nom qui n existe pas', nomInconnu.corps.inconnu === true,
   JSON.stringify(nomInconnu.corps));

// -------------------------------------------------- Instagram comme preuve
console.log('\n── INSTAGRAM COMME PREUVE ───────────────────────────────────');
const NOM_IG = `Insta ${marque}`.slice(0, 20);
const telIg = appareil('ig1');
const claimIg = await post('/claim', { device_id: telIg, name: NOM_IG });
const CODE_IG = claimIg.corps.code;
ok('le nom Instagram se reserve', claimIg.corps.ok === true, JSON.stringify(claimIg.corps));

const profil = await post('/profil', { device_id: telIg, name: NOM_IG, insta: '@le_compte_joueur' });
...
ok('il dit de quel compte ecrire', demandeIg.corps.insta === 'le_compte_joueur',
...
ok('le mot de passage se retrouve apres avoir ferme le jeu',
   relu.corps.phrase === demandeIg.corps.phrase && relu.corps.phrase != null,
   String(relu.corps.phrase));
...
ok('la demande Instagram porte les deux moities de la preuve',
   igEnFile && igEnFile.insta === 'le_compte_joueur'
