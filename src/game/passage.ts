...
export function cameraPassage(maintenant: number, VW: number, VH: number): Camera | null {
  ...
  if (p.duree === DUREE_DOUCE) {
    return { u, sens: p.sens, axe: p.axe, doux: true,
             part: p.sens === 'aller' ? u : 1 - u,
             dx: 0, dy: 0, roll: 0, zoom: 1, fond, voile: adoucir(u) * 0.92 };
  }
  ...
  if (p.axe === 'bas') {
    // Vers les haies, on DESCEND : le stade sort par le haut, et l'oeil se
    // rapproche du sol — c'est ce leger rapprochement qui fait la difference
    // entre une camera qui descend et une image qui glisse.
    dy = -VH * 1.05 * part;
    // L'oeil RECULE avant de plonger. Sans ce retrait, une seule haie tenait
    // dans le cadre : a l'echelle du jeu, neuf metres quatorze mangent la
    // moitie de l'ecran, et on ne voyait pas le peigne se monter. On recule
    // pendant que les haies sortent, on avance une fois qu'elles sont la.
  ...
  const voile = 0.92 * Math.pow(part, 2.2);

  return { u, sens: p.sens, axe: p.axe, doux: false, part,
