...
import {
  fetchLeaderboardRaw, onLeaderboard, rankByRaceTime,
 ...
 * occuper plusieurs lignes (plusieurs appareils, plusieurs records). Sur la
 * piste il n'y a qu'un couloir par coureur — on ne retient donc que la
 * premiere apparition de chaque nom, et la place liberee revient au suivant
 * au classement. Si le TOP 500 ne fournit pas sept noms, on complete avec les
 * coureurs de l'etape, ceux qui existent deja dans le jeu.
 */
export function buildField(
  ...
  for (const e of rankByRaceTime(entries)) take(e.name);
  for (const n of fallback) take(n);
  return field;
}
  ...
  try {
    ...
    for (const race of RACES) {
      ...
      if (Array.isArray(names) && names.length) {
        SprinterApp.G.top500Field[race] = names.slice(0, FIELD_SIZE).map(String);
      }
    }
  } catch (e) { }
}
