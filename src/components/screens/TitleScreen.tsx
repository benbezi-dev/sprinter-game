...
import { OneShotPanel, ChallengePanel, PLUSIEURS_DEFIS } from './ModePanels';
...
import { useResteDessous } from '@/hooks/use-reste-dessous';
...
export function TitleScreen() {
  ...
  // Reste-t-il de la page dessous ? La reponse commande la fleche du bas.
  const resteDessous = useResteDessous(rouleau);
  const descendre = () => {
    const el = rouleau.current;
    if (!el) return;
    // Une page, moins un peu : on garde a l'ecran la derniere ligne de ce
    // qu'on lisait, sinon on ne sait plus d'ou l'on vient.
  ...
  return (
    <div className="relative w-full h-full pointer-events-auto">
              ...
              <div className="bg-card/60 backdrop-blur-sm border border-white/10 px-6 py-5 md:px-8 md:py-6 rounded-2xl w-full max-w-md border-t-white/20">
                ...
                <p className="mt-1 md:mt-2 text-[10px] sm:text-xs md:text-base lg:text-xl font-medium text-foreground/80 tracking-wide uppercase">
                  {tab === 'career'
                    ...
                    : N.t(tab === 'oneshot' ? 'oneshot_desc'
                          : PLUSIEURS_DEFIS ? 'versus_family' : 'versus_desc')}
                </p>
              </div>
            </div>
      ...
      {/* Il reste de la page, et rien ne le disait.
          Qui connait le jeu fait glisser sans y penser ; qui l'ouvre pour la
          premiere fois voit un ecran plein et en conclut qu'il a tout vu —
          c'est ainsi qu'on passe a cote d'une facon de jouer entiere. La
          fleche ne parait que s'il reste vraiment quelque chose dessous et
          s'efface d'elle-meme au bout : elle ne dit jamais rien de faux. On
          peut la toucher autant que faire glisser, parce que le premier
          reflexe, devant une fleche, est d'appuyer dessus. */}
      {resteDessous && (
        <button
          onClick={descendre}
          aria-label={N.t('plus_bas')}
          title={N.t('plus_bas')}
          className="absolute left-1/2 -translate-x-1/2 z-30
                     bottom-[max(env(safe-area-inset-bottom),0.35rem)]
                     px-2.5 py-1 rounded-full bg-black/75 backdrop-blur-md border border-white/15
                     text-primary shadow-lg hover:bg-black/90 transition-colors"
        >
