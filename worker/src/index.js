...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/claim' && request.method === 'POST') {
      ...
      //
      // La table des scores est garantie avant d'etre lue. Elle ne l'etait pas,
      // et sur une base ou personne n'avait encore pose de chrono — une base
      // neuve, celle du canal de test — reserver un nom echouait en 500 sur un
      // « no such table: scores ». En production la table existait depuis le
      // premier jour, si bien que le defaut ne se voyait jamais la ou on
      // regardait.
      await ensureScoreGhost(env.DB);
      await env.DB.prepare(
